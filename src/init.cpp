...
void Shutdown()
{
    ...
    RenameThread("piratescoin-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/piratescoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to piratescoind / RPC client
            std::string strUsage = _("Piratescoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "piratescoin:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect piratescoind signal handlers
    noui_connect();
...
{
    RenameThread("piratescoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Piratescoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "Piratescoin server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Piratescoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Piratescoin to complete") << "\n";

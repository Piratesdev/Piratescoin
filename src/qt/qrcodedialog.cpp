...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("PTE")
    updateDisplayUnit();
}
...
{
    QString ret = QString("piratescoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PTE unit input in lnReqAmount, we generate the URI with PTE as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PTE, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
